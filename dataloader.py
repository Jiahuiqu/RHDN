...
class HS(Dataset):
    ...
    def __getitem__(self, item):
        ...
        data_img1 = np.squeeze(sio.loadmat(img1)['da'].astype(np.float32))
        data_img2 = sio.loadmat(img2)['da'].astype(np.float32)
        data_img2 = np.reshape(data_img2, [1, 160, 160])
        data_label = np.squeeze(sio.loadmat(label)['da'].astype(np.float32))
